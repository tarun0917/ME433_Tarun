...
void APP_Initialize(void) {
    ...
    //Initialise the pins as Output Compares
    RPA0Rbits.RPA0R = 0b0101; // A0 to OC1
    RPB13Rbits.RPB13R = 0b0101; // B13 to OC4
    
    //Configure the Output Compares
        ...
        case APP_STATE_SCHEDULE_READ:
         ...
         if (APP_StateReset()) {
            ...
            if (appData.isReadComplete == true) {
                ...
                while (appData.readBuffer[ii] != 0) {
                    ...
                    if (appData.readBuffer[ii] == '\n' || appData.readBuffer[ii] == '\r') {
                        ...
                        sscanf(rx, "%d %d",&rxVal,&Kp); // get the int out of the array
            ...
            error = rxVal - 240; // 240 means the dot is in the middle of the screen
                    if (error<-225){ // slow down the right motor to steer to the right
                    ...
                    else if (error<-125&& error>-225){ // slow down the right motor to steer to the right
                        // MAX_DUTY = 400;
                       error  = -error;
                        left = MAX_DUTY - Kp*error;
                        right = MAX_DUTY;
                     ...
                     else if (error<0 && error>-125){ // slow down the right motor to steer to the right
                        // MAX_DUTY = 400;
                         error  = -error;
                       left = 800 - Kp*error;
                        right = 800;
                     ...
                     else if (error>0 && error<125){ // slow down the left motor to steer to the left
                        // MAX_DUTY = 400;
                        right = 800 - Kp*error;
                        left = 800;
                        if (right<0) {
                     ...
                     else if (error>125 && error<225){ // slow down the left motor to steer to the left
                         //MAX_DUTY = 400;
                     ...
                     else if (error>225){ // slow down the left motor to steer to the left
                        // MAX_DUTY = 400;
                        right = MAX_DUTY - Kp*error;
                        left = MAX_DUTY;
                        if (right<0) {
        ...
        case APP_STATE_CHECK_TIMER:
             ...
             * The isReadComplete flag gets updated in the CDC event handler. */
             ...
             /* WAIT FOR 5HZ TO PASS OR UNTIL A LETTER IS RECEIVED */
              if (gotRx || _CP0_GET_COUNT() - startTime > (48000000 / 2 /15)) {
